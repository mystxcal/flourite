...
state, commits only if that projection succeeds, and then atomically replaces
`state.json`. A projection error leaves neither a stray event nor a mismatched
snapshot.
